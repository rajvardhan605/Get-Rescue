...
**Needs redeploying**: `dist/driverTicket.zip` (re-packed).
...
1. `alreadySuccess` was reading the shared `Payment_Status` field (also used by the agent's Booking Fee stage), which could bypass the mandatory-photo gate before the driver had done anything. Now checks `Payment_received==="Yes"` — written only by this screen's own action.
