...
| `Schedule_Date_Time` | Schedule Date & Time | Date-Time | — | ✅ **Confirmed current schema (2026-08-05 replacement)** — corrects this table's own earlier `Schedule_Date` (Date) + `Service_Time` (half-hour Dropdown) two-field design, which was superseded by a single combined Date-Time field the same week; no code anywhere in this project references `Schedule_Date`/`Service_Time` any more (confirmed via a full-application audit, 2026-09-12). Required when `Time_of_service = LATER`; one native `datetime-local` picker writes straight into it |
| `Computed_Service_Time` | Computed Service Time | Date-Time | — | Hidden; auto-computed (NOW → +30 min from save time; LATER → picked `Schedule_Date_Time`) |
...
| `Client_Charge` | Client Charge | Number | — | ✅ **Fully wired** (correction 2026-09-12 audit — this row previously said "not yet wired into the widget," which is stale; it's editable in the `quote` stage, drives `Rescue_Charge`'s own recompute, and has its own 2026-08-25 bug fix). Editable only when `Client` ≠ the special `"ON DEMAND"` walk-in value; null/disabled when it is |
...
| `Payment_received` | Payment Received? | Radio | `Yes`, `No` | Same 2026-09-02 always-editable-from-Closure fix as `Payment_Method_Final` above. **Also set automatically to `Yes` by `paymentWebhook` (2026-09-14)** when the customer pays the Remaining Fee link online — the agent no longer has to tick it by hand, though it stays agent-editable for cash/offline payments |
| `Remaining_Fee_Transaction_ID` | RF Transaction ID | Single Line | — | ✅ **Created in Zoho 2026-09-14 (user-confirmed)** and written by `paymentWebhook` on a Remaining Fee (`-RF`) payment — the sibling of `Booking_Fee_Transaction_ID` above, sourced from the real Zoho Payments webhook rather than typed by an agent. This is the field [FIELDS.md's own Step 8 row](#) previously recorded as unimplementable ("no real gateway webhook exists to source these from honestly") — that webhook now exists |
...
| `Zoho_Books_Invoice_URL` | Zoho Books Invoice URL | Single Line | — | ✅ **Created in Zoho, user-confirmed 2026-09-11** (Task Tracker PDF item 27, "Invoice Link"). `onDemandInvoicing()` already returns a real invoice/payment URL from Zoho Books — this field is where that button's own call (`Generate_OnDemand_Invoice`) writes it, via that function's existing `paymentUrlField` parameter. Shown via `__OnDemandInvoiceLinkDisplay` at the end of the Closure page. **Changed 2026-09-17 (explicit user decision, "do not need to login book switch to a shareable link")**: this now receives Zoho Books' own **public** `invoice_url` — the customer-facing link that opens with no Zoho Books login — instead of the internal `books.zoho.in/invoices/<id>` app screen, which only opened for someone already signed into Books with access to the org. The internal URL remains the fallback if Books returns no public URL, and the execution log says which was used. ⚠ The public URL only exists once the **Client Portal is enabled** for the organization in Zoho Books (Settings → Portal) — if the log reports "no public invoice_url came back", that's the setting to check. **Not yet live-tested end-to-end** — worth clicking "Generate Invoice" on a real ON DEMAND ticket to confirm this field populates and the link opens in a signed-out browser |
...
| `Cash_Collected` | Cash Collected | Number | ✅ **Wired 2026-09-14** (was "no writer anywhere in this app"). Per the PDF's own "Case System, not editable" source column it is DERIVED, never typed: `cashCollectedForTicket()` returns `Remaining_Fee` when `Payment_Method_Final==="Cash"` **and** `Payment_received==="Yes"`, else 0 — i.e. the remaining fee the customer settled in cash on site, which the vendor is therefore already holding. Shown and deducted on the Level 1 (Operations Approval) screen, then **written onto the case by `saveVendorAcctApproval()`** so Level 2 (Accounts Payable) and the Zoho Books vendor credit all pay from the same approved figure. Deliberately excludes the Booking Fee (collected before any vendor is assigned, and `Payment_Method` defaults to "Cash" on every ticket — counting it would underpay nearly every vendor) and the non-Cash methods (UPI/Card/Net Banking may have landed in the company's own account) |
...
| `Settlement_Status` | Settlement Status | Dropdown | **Value list confirmed 2026-09-04, field existence user-confirmed 2026-09-17**: `Pending Approval` (or blank) → `Payable` → `Paid`, matching the PDF's own Case-completed → Operations-Approved → Payment-completed flow. Named constants (`SETTLEMENT_STATUS_PENDING`/`_PAYABLE`/`_PAID`) in `getRescueTicket/app/widget.html` make a 1-line fix if the live dropdown's wording turns out to differ — the symptom to watch for is Zoho rejecting an approval or payment with "Invalid column value for Settlement_Status". The Level 1 screen sets this to `Payable` on approval; **Level 2 advances it to `Paid` on payment (added 2026-09-17** — it previously stopped at `Payable` forever, so no case ever reached its terminal state). A case only appears in the pending-approval list while this is blank or `Pending Approval`. The PDF's own wording for the final state is "PAYMENT COMPLETED"; `Paid` is used because that is what this project's confirmed dropdown contains |
...
### Level 2 (Accounts Payable) fields — ✅ ALL CREATED, user-confirmed 2026-09-17

Fields for the batch vendor-payment screen (`accountsPayableBtn`/`recordVendorPayment.deluge`, shipped 2026-09-04). **All nine below now exist in Zoho** (user-confirmed 2026-09-17), which unblocks the Accounts Payable screen end-to-end — it was previously unable to record a payment at all.

**Still never executed live**, which is now the only thing standing between this and done: no vendor payment run has ever been made, so the Zoho Books Bill + Vendor Payment + Vendor Credit chain in `recordVendorPayment.deluge` remains researched-but-untested against the real API (its own header comment says the same). Expect one live bug-fix round, exactly as `onDemandInvoicing.deluge` needed. Both Books account ids it depends on **are** configured: `EXPENSE_ACCOUNT_ID` = `3988332000000174002` (Vendor Service Charges), `PAID_THROUGH_ACCOUNT_ID` = `3988332000000000459` (Petty Cash).
...
| `Fleet_ID` | Fleet ID | Single Line | — | 🟢 **User-confirmed 2026-09-17, wired the same day.** The vendor's own fleet identifier — the "vendor payments" PDF's `Vendor ID` column ("Display the Fleet ID from Vendor table"), shown on **both** Vendor Accounting screens and used as the Accounts Payable vendor search key ("VENDOR fleet id - ALLOW TO type and search for fleet id"). Read via `fleetIdOfVendorRaw()` in `getRescueTicket/app/widget.html`, which also accepts the casing variants `Fleet_Id`/`FleetID`/`fleet_id` as insurance — this account has twice used a lowercase API name where the documented one was capitalised (`service_type`, `technician_name`). A vendor with this blank shows `—`; there is deliberately **no** fallback to the Zoho record ID, which would print a 19-digit number that reads as a real fleet identifier and is not one |
| `Current_Latitude` / `Current_Longitude` | Current Latitude / Longitude | Single Line | — | ✅ **Created in Zoho (user-confirmed 2026-09-25)** — see the cross-cutting `Vendors_Report` section further down for the full writeup (live-location heartbeat, `vendorTicket`) |
...
| `Current_Latitude` / `Current_Longitude` | Current Latitude / Longitude | Single Line | — | ✅ **Created in Zoho (user-confirmed 2026-09-25)** — mirrors `Vendors_Report`'s own pair, see the cross-cutting `Technicians_Report` section further down |
...
| `Schedule_Date_Time` | Schedule Date & Time (when `LATER`) — single Date-Time picker, replaced the earlier separate `Schedule_Date`+`Service_Time` design | datetime | Agent | ✅ (confirmed current in code, 2026-09-12 audit) |
| `Computed_Service_Time` | Computed service time (NOW = +30min, LATER = `Schedule_Date_Time`), 12-hour+AM/PM+seconds format | (hidden, `extra()`) | Agent | ✅ (format confirmed live) |
...
| `Payment_Method_Final` | Payment Method — same option list as Step 2's own `Payment_Method`, but tracked as a separate field. **Renamed 2026-08-03 from `Payment_Method1`** | select | Agent, Technician, Driver, Vendor | ✅ — **corrected 2026-09-13, this row was stale**: the 2026-08-03 confirmation (`["Cash","UPI","Card","Net Banking"]`) was superseded the very next day — "Payment Gateway" was restored as a 5th option on 2026-08-04 (user request), and is live in the dropdown in `getRescueTicket`/`vendorTicket`/`technicianTicket`/`driverTicket` today. It remains a decorative placeholder — no real gateway/webhook is wired up — see `QR_Code`'s own corrected row below |
| `Payment_received` | Payment Received? | radio | Agent, Technician, Driver, Vendor | 🟡 |
| `QR_Code` | QR / Reference | text | Agent | 🟡 — **corrected 2026-09-13, this row was wrong**: it previously said no widget shows a QR block anymore. All of `vendorTicket`/`technicianTicket`/`driverTicket`'s own Final Payment screens DO show a QR block when `Payment_Method_Final==="Payment Gateway"` is selected (a static decorative pattern, not a real generated QR image) — restored alongside the option itself on 2026-08-04. The `QR_Code` **field** itself does still appear genuinely unused by any widget (nothing reads/writes it) — that part of the old note was correct, just not the "no QR block" part |
| `Send_Payment_Link1` | Send payment link | check | Agent | 🟡 |
| `Upload_Photo_of_receipt` | Label "Remaining Fee Receipt Photo" as of 2026-09-02 (build-plan item #8, agent side only — see that field's own note) — mandatory for every Payment Method now — both a live-camera capture (GPS/time watermark) and a plain gallery picker are offered | file | Agent, Technician, Driver, Vendor | 🟡 — **note, 2026-09-13**: "every Payment Method" here is true for the 4 real ones (Cash/UPI/Card/Net Banking); `Payment_Method_Final==="Payment Gateway"` is still a deliberate carve-out from the mandatory-photo gate in `vendorTicket`/`technicianTicket`/`driverTicket` (`success = alreadySuccess \|\| (!gateway && receiptOk)`) — see that field's own corrected row above |
